...
class TiaTacToeVC: UIViewController{
...
extension TiaTacToeVC: UICollectionViewDataSource, UICollectionViewDelegate {
    
    private func setupCollectionView() {
        myCollectionView.register(TiaTacToeCell.self, forCellWithReuseIdentifier: "TiaTacToeCell")
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "TiaTacToeCell", for: indexPath) as! TiaTacToeCell
    ...
    private func announceWinner(player: String) {
        let alert = UIAlertController(title: "Game over!", message: "\(player) Congratulations you won", preferredStyle: .alert)
        alert.addAction(UIAlertAction(title: "Dismiss", style: .cancel, handler: { [weak self] _ in
            self?.resetState()
        }))
        DispatchQueue.main.async {
